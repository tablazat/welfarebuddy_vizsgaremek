...
import { Heart, Activity, Weight, Plus, Loader2, Clock, Crown, Lock, Pencil, Trash2, Flame, Droplet, Moon, Star, CalendarIcon, ChevronLeft, ChevronRight } from "lucide-vue-next"
...
const cForm = ref({ data: "", recorded_at: nowLocal() })
...
const filteredC = computed(() => filterByQuery(calorieItems.value, ["data", "recorded_at"]))
const filteredWater = computed(() => filterByQuery(waterItems.value, ["amount_ml", "recorded_at"]))
const filteredSleep = computed(() => filterByQuery(sleepItems.value, ["hours", "recorded_at"]))

async function loadAll() {
  loading.value = true
  try {
    const params = { start_date: todayStr.value, end_date: tomorrowStr.value }
    const [hrRes, bpRes, wRes, cRes, waterRes, sleepRes] = await Promise.allSettled([
    ...
    if (cRes.status === "fulfilled") calorieItems.value = cRes.value.data.slice().reverse()
    if (waterRes.status === "fulfilled") waterItems.value = waterRes.value.data.slice().reverse()
    if (sleepRes.status === "fulfilled") sleepItems.value = sleepRes.value.data.slice().reverse()
    const results = [hrRes, bpRes, wRes, cRes, waterRes, sleepRes]
...
watch(wsEvent, (e) => {
  ...
  switch (e.type) {
    ...
    case "weight":
      ...
      break
    case "calorie":
      if (entry?.data) calorieItems.value.unshift(entry)
      break
    case "water":
      if (entry?.amount_ml) waterItems.value.unshift(entry)
      break
    case "sleep":
      if (entry?.hours) sleepItems.value.unshift(entry)
      break
    default:
      loadAll()
  }
...
async function saveCalorie() {
  clearMessages()
  if (!cForm.value.data) { error.value = t("diary.validation.calorie"); return }
  saving.value = true
  try {
    const payload = { data: Number(cForm.value.data) }
    if (isPro.value && cForm.value.recorded_at) payload.recorded_at = cForm.value.recorded_at
    await api.post("/calories", payload)
    cForm.value = { data: "", recorded_at: nowLocal() }
...
const deleteEndpoints = {
  heartRate: (id) => `/heart-rates/${id}`,
  bloodPressure: (id) => `/blood-pressures/${id}`,
  weight: (id) => `/weights/${id}`,
  calorie: (id) => `/calories/${id}`,
  water: (id) => `/waters/${id}`,
  sleep: (id) => `/sleeps/${id}`,
}
...
async function deleteItem(type, id) {
  ...
  deletingId.value = id
  try {
    await api.delete(deleteEndpoints[type](id))
    await loadAll()
  } catch (e) {
    error.value = getAxiosErrorMessage(e)
  } finally {
...
function openEdit(type, item) {
  ...
  if (type === "heartRate") {
    editForm.value = { heart_rate: item.heart_rate, context: item.context || "", recorded_at: recordedAt }
  } else if (type === "bloodPressure") {
    editForm.value = { systolic: item.systolic, diastolic: item.diastolic, period: item.period || "", recorded_at: recordedAt }
  } else if (type === "weight") {
    editForm.value = { weight: item.weight, recorded_at: recordedAt }
  } else if (type === "calorie") {
    editForm.value = { data: item.data, recorded_at: recordedAt }
  } else if (type === "water") {
    editForm.value = { amount_ml: item.amount_ml, recorded_at: recordedAt }
  } else if (type === "sleep") {
    editForm.value = { hours: item.hours, quality: item.quality || "", recorded_at: recordedAt }
  }
...
async function saveEdit() {
  ...
  try {
    ...
    await api.put(editEndpoints[editType.value](editItem.value.id), payload)
    showEditModal.value = false
    const savedKey = editType.value === "heartRate" ? "heartRate"
      : editType.value === "bloodPressure" ? "bloodPressure"
      : editType.value === "weight" ? "weight"
      : editType.value === "water" ? "water"
      : editType.value === "sleep" ? "sleep"
      : "calorie"
    successMsg.value = t(`diary.saved.${savedKey}`)
    await loadAll()
  } catch (e) {
    error.value = getAxiosErrorMessage(e)
  } finally {
      ...
      <div class="flex items-center gap-1.5">
        ...
        <Popover v-model:open="datePickerOpen">
          <PopoverTrigger as-child>
            <Button type="button" variant="outline" class="h-9 flex-1 sm:flex-none sm:min-w-[240px] justify-start gap-2 rounded-xl font-normal">
    ...
    <Tabs v-model="activeTab" class="w-full">
      <TabsList class="grid grid-cols-6 w-full rounded-xl">
        ...
        <TabsTrigger value="calorie" class="gap-1.5">
          ...
          <span class="hidden sm:inline">{{ $t("diary.calorie") }}</span>
          <span class="sm:hidden">{{ $t("diary.calorieShort") }}</span>
        </TabsTrigger>
        <TabsTrigger value="water" class="gap-1.5">
          <Droplet class="h-3.5 w-3.5" />
          <span class="hidden sm:inline">{{ $t("diary.water") }}</span>
          <span class="sm:hidden">{{ $t("diary.waterShort") }}</span>
        </TabsTrigger>
        <TabsTrigger value="sleep" class="gap-1.5">
          <Moon class="h-3.5 w-3.5" />
          <span class="hidden sm:inline">{{ $t("diary.sleep") }}</span>
          <span class="sm:hidden">{{ $t("diary.sleepShort") }}</span>
        </TabsTrigger>
      </TabsList>
        ...
        <div v-else-if="filteredHR.length" class="space-y-2">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              ...
              <button type="button" class="text-muted-foreground hover:text-foreground p-1 rounded" @click="openEdit('heartRate', item)">
                <Pencil class="h-3.5 w-3.5" />
              </button>
              <button type="button" class="text-muted-foreground hover:text-destructive p-1 rounded" :disabled="deletingId === item.id" @click="deleteItem('heartRate', item.id)">
        ...
        <div v-else-if="filteredBP.length" class="space-y-2">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              ...
              <button type="button" class="text-muted-foreground hover:text-foreground p-1 rounded" @click="openEdit('bloodPressure', item)">
                <Pencil class="h-3.5 w-3.5" />
              </button>
              <button type="button" class="text-muted-foreground hover:text-destructive p-1 rounded" :disabled="deletingId === item.id" @click="deleteItem('bloodPressure', item.id)">
                <Loader2 v-if="deletingId === item.id" class="h-3.5 w-3.5 animate-spin" />
                <Trash2 v-else class="h-3.5 w-3.5" />
              </button>
            </div>
      ...
      <TabsContent value="weight" class="mt-4 space-y-4">
        ...
        <div v-else-if="filteredW.length" class="space-y-2">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              <button type="button" class="text-muted-foreground hover:text-foreground p-1 rounded" @click="openEdit('weight', item)">
                <Pencil class="h-3.5 w-3.5" />
              </button>
              <button type="button" class="text-muted-foreground hover:text-destructive p-1 rounded" :disabled="deletingId === item.id" @click="deleteItem('weight', item.id)">
                <Loader2 v-if="deletingId === item.id" class="h-3.5 w-3.5 animate-spin" />
                <Trash2 v-else class="h-3.5 w-3.5" />
              </button>
            </div>
      ...
      <TabsContent value="calorie" class="mt-4 space-y-4">
            ...
            <form class="space-y-3" @submit.prevent="saveCalorie">
              <div class="flex gap-3 flex-wrap">
                <div class="space-y-1.5 flex-1 min-w-[120px]">
                  ...
                  <Input v-model="cForm.data" type="number" min="1" max="10000" placeholder="500" />
                </div>
                <div class="space-y-1.5 flex-1 min-w-[180px]">
                  <Label class="flex items-center gap-1.5">
                    {{ $t("diary.dateTimeOptional") }}
                  ...
                  <Input v-else v-model="cForm.recorded_at" type="datetime-local" :max="nowLocal()" />
                </div>
              </div>
              <Button type="submit" class="gap-2 rounded-xl" :disabled="saving">
                <Loader2 v-if="saving" class="h-4 w-4 animate-spin" />
        ...
        <div v-else-if="filteredC.length" class="space-y-2">
          <div
            v-for="item in filteredC"
            :key="item.id"
            class="flex items-center justify-between rounded-xl border bg-card px-4 py-3"
          >
            <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
              <button type="button" class="text-muted-foreground hover:text-foreground p-1 rounded" @click="openEdit('calorie', item)">
                <Pencil class="h-3.5 w-3.5" />
              </button>
              <button type="button" class="text-muted-foreground hover:text-destructive p-1 rounded" :disabled="deletingId === item.id" @click="deleteItem('calorie', item.id)">
                <Loader2 v-if="deletingId === item.id" class="h-3.5 w-3.5 animate-spin" />
                <Trash2 v-else class="h-3.5 w-3.5" />
              </button>
            </div>
      ...
      <TabsContent value="water" class="mt-4 space-y-4">
        ...
        <div v-else-if="filteredWater.length" class="space-y-2">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              <button type="button" class="text-muted-foreground hover:text-foreground p-1 rounded" @click="openEdit('water', item)">
                <Pencil class="h-3.5 w-3.5" />
              </button>
              <button type="button" class="text-muted-foreground hover:text-destructive p-1 rounded" :disabled="deletingId === item.id" @click="deleteItem('water', item.id)">
                <Loader2 v-if="deletingId === item.id" class="h-3.5 w-3.5 animate-spin" />
                <Trash2 v-else class="h-3.5 w-3.5" />
              </button>
            </div>
      ...
      <TabsContent value="sleep" class="mt-4 space-y-4">
        ...
        <div v-else-if="filteredSleep.length" class="space-y-2">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              <button type="button" class="text-muted-foreground hover:text-foreground p-1 rounded" @click="openEdit('sleep', item)">
                <Pencil class="h-3.5 w-3.5" />
              </button>
              <button type="button" class="text-muted-foreground hover:text-destructive p-1 rounded" :disabled="deletingId === item.id" @click="deleteItem('sleep', item.id)">
                <Loader2 v-if="deletingId === item.id" class="h-3.5 w-3.5 animate-spin" />
                <Trash2 v-else class="h-3.5 w-3.5" />
              </button>
            </div>
    ...
    <Teleport to="body">
      <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" @click.self="showEditModal = false">
        <div class="bg-background rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
          <h3 class="font-semibold text-base">{{ $t("diary.editTitle") }}</h3>
          <form class="space-y-3" @submit.prevent="saveEdit">
            <template v-if="editType === 'heartRate'">
              <div class="space-y-1.5">
                <Label>{{ $t("diary.hrLabel") }}</Label>
            ...
            <template v-else-if="editType === 'weight'">
              <div class="space-y-1.5">
                <Label>{{ $t("diary.weightLabel") }}</Label>
                <Input v-model="editForm.weight" type="number" min="1" max="500" step="0.1" class="rounded-xl" />
              </div>
            </template>
            ...
            <div class="flex gap-2 pt-1">
              <Button type="submit" class="gap-2 rounded-xl flex-1" :disabled="editSaving">
                <Loader2 v-if="editSaving" class="h-4 w-4 animate-spin" />
                {{ $t("common.save") }}
              </Button>
              <Button type="button" variant="outline" class="rounded-xl" @click="showEditModal = false">
                {{ $t("common.cancel") }}
              </Button>
            </div>
          </form>
