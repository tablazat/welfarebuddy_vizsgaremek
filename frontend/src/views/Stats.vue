  ...
  calorieChart = new Chart(calorieCanvas.value, {
    ...
    data: {
      ...
      datasets: [{
        ...
        data: calorieData.value.map(d => d.data),
    ...
    <Card v-if="!loading && heatmapCells.length >= 7" class="rounded-2xl">
      ...
      <CardContent>
        <div class="overflow-x-auto pb-2">
          ...
          >
            <template v-for="(cell, idx) in heatmapCells" :key="cell.empty ? `e-${idx}` : cell.date">
              <div v-if="cell.empty" class="w-3.5 h-3.5" />
              <div
                v-else
        ...
        <div class="mt-3 flex items-center justify-end gap-1.5 text-xs text-muted-foreground">
          ...
          <div class="w-3.5 h-3.5 rounded-sm bg-muted"></div>
          <div class="w-3.5 h-3.5 rounded-sm bg-emerald-200 dark:bg-emerald-900"></div>
          <div class="w-3.5 h-3.5 rounded-sm bg-emerald-400 dark:bg-emerald-700"></div>
          <div class="w-3.5 h-3.5 rounded-sm bg-emerald-500"></div>
          <div class="w-3.5 h-3.5 rounded-sm bg-emerald-600 dark:bg-emerald-400"></div>
