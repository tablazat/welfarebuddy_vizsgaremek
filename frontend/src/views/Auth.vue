  ...
  <div class="min-h-svh flex items-center justify-center bg-muted/40 p-4">
    <div class="w-full max-w-md">
      <div class="mb-4 flex items-center justify-between">
        <Button variant="ghost" size="sm" class="rounded-xl gap-1.5 text-muted-foreground" @click="router.push('/')">
      ...
      <div class="text-center mb-6">
        ...
        <p class="text-muted-foreground text-sm mt-1">{{ $t("landing.heroSubtitle") }}</p>
      </div>
            ...
            <TabsContent value="register" class="mt-4">
              <form class="space-y-4" @submit.prevent="register">
                ...
                <label class="flex items-start gap-2 text-xs text-muted-foreground cursor-pointer">
                  ...
                  <span>
                    {{ $t("auth.termsPrefix") }}
                    <router-link to="/legal?tab=tos" target="_blank" class="underline text-foreground">{{ $t("auth.termsLinkTos") }}</router-link>{{ $t("auth.termsSep1") }}
                    <router-link to="/legal?tab=gdpr" target="_blank" class="underline text-foreground">{{ $t("auth.termsLinkGdpr") }}</router-link>{{ $t("auth.termsSep2") }}
                    <router-link to="/legal?tab=eula" target="_blank" class="underline text-foreground">{{ $t("auth.termsLinkEula") }}</router-link>{{ $t("auth.termsSuffix") }}
                  </span>
                </label>
